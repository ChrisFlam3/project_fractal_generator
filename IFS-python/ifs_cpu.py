...
#class for flam3 execution and data postprocessing using cpu
class ifs_cpu():
                ...
                for a in range(5):
                    for b in range(5):
                        ...
                        if (freq != 0 and maxv>1):
                            alpha += color*pow((math.log(freq,10) / math.log(maxv,10)), 1 / 2.2)
                            #print(alpha)
                        freqArchive[y][i] += histogram[5 * y + b][5 * i + a][0]#histogram[y][i][0] pass on
    ...
    #render to file or overview using cpu
    def render_one_core(self,overview=0):
        ...
        cad=0
        histogram=np.zeros((resolution,resolution,2))#[[[0 for i in range(2)] for k in range(10241)] for j in range(10241)]
        ...
        for i in range(i_count):
            ...
            point=affineMatrix[idx].transform_point(point)
            cad = (cad + self.ifs_general.affineTransforms[idx][6]) / 2 
            point=unlinearPointer[self.ifs_general.current[idx]](point) 
            point=postMatrix[idx].transform_point(point)
            ...
            if i>20:
                if (abs(point.x) > 5 * self.ifs_general.zoom or abs(point.y) > 5 * self.ifs_general.zoom):
                    continue
                temp =int((point.x + 5 * self.ifs_general.zoom) * ((resolution-1)/10 / self.ifs_general.zoom))
                temp2 =int((point.y + 5 * self.ifs_general.zoom) * ((resolution-1)/10 / self.ifs_general.zoom))
        ...
        #if rendering to overview skip all postprocessing methods
        if(overview==0):
            self.post_process_one_core(histogram,maxv)
        else:
            img=sf.Image.create(500,500)
            freq = 0
            color = 0
            alpha = 0
            ...
            for i in range(resolution-1):
                for y in range(resolution-1):
                    ...
                    color = histogram[y][i][1]
                    if (freq != 0):
                        alpha = color*pow((math.log(freq) / math.log(maxv)), 1 / 2.2)#gamma normalization
                    else:
                        alpha = 0
